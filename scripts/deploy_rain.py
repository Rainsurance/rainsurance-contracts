...
def deploy(
    ...
    rainProduct = rainDeploy.getProduct()
    rainOracle = rainProduct.getOracle()
    rainRiskpool = rainProduct.getRiskpool()

    product = rainProduct.getContract()
    oracle = rainOracle.getContract()
    riskpool = rainRiskpool.getContract()
    ...
    riskId1 = tx[0].events['LogRainRiskDataCreated']['riskId']
    riskId2 = tx[1].events['LogRainRiskDataCreated']['riskId']
    ...
    processId1 = tx[0].events['LogRainPolicyCreated']['policyId']
    processId2 = tx[1].events['LogRainPolicyCreated']['policyId']
    ...
    deploy_result = {
        ...
        PRODUCT: contract_from_address(RainProduct, product),
        ORACLE: contract_from_address(RainOracle, oracle),
        RISKPOOL: contract_from_address(RainRiskpool, riskpool),
        RISK_ID1: riskId1,
        RISK_ID2: riskId2,
        PROCESS_ID1: processId1,
        PROCESS_ID2: processId2,
...
def from_registry(
    ...
    if products >= 1:
        ...
        product = contract_from_address(RainProduct, componentAddress)
    ...
    if oracles >= 1:
        ...
        componentAddress = instanceService.getComponent(componentId)
        oracle = contract_from_address(RainOracle, componentAddress)
    ...
    if riskpools >= 1:
        ...
        componentAddress = instanceService.getComponent(componentId)
        riskpool = contract_from_address(RainRiskpool, componentAddress)
